...
{
	public:
		NRF24L01(byte csn, byte ce, byte irq);
		NRF24L01(void);
		void trans_init();
		void rx_config(byte pipe_num, byte *pipe_addr, byte pipe_addr_width, byte rx_pload_width);
		void tx_config(byte *tx_addr, byte tx_addr_width);
		void rx();
		void tx(byte *tx_buf, byte buf_size);
		byte check_rx(byte *buf, byte rx_pload_width);
		byte check_tx_flag();

		void rx_test_config(byte buf_size);
		byte rx_packet(byte *p_buf, byte size);
		void tx_test_config(byte buf_size);
		void tx_test(byte *tx_buf, byte *rx_buf, byte buf_size);
	private:
		void chip_select();
		void chip_deselect();
		...
		byte write_buf(byte addr, byte *p_buf, byte n_bytes);
		byte irq() { return !digitalRead(_pin_IRQ); }
	private:
